...
# IO500 Reproducibility - SC25 - HPE - DAOS on ProLiant DL360 with NDR InfiniBand
...
HPE Cray Supercomputing Storage Benchmark Center in Fort Collins, Colorado, USA.
...
This cluster is available to HPE engineers and HPE business partners for performance testing
...
The storage software is Distributed Asynchronous Object Store (DAOS), currently at Version 2.6.4-6.
...
The DAOS storage system for this IO500 submission is a **single** HPE ProLiant DL360 Gen12 server,
currently running DAOS Version 2.6.4-6 on Rocky Linux version 9.5:

* 2x Intel Xeon Gold 6740P (48 cores)
* 2048 GiB of DRAM memory
* 20x Samsung PM1743 15.36 TB E3.S NVMe SSDs
* 2x ConnectX-7 NDR NICs
...
[https://github.com/daos-stack/daos-reproducibility/tree/master/io500/sc25/hpe/dl360-ndr](https://github.com/daos-stack/daos-reproducibility/tree/master/io500/sc25/hpe/dl360-ndr)
...
```
    [ior-easy]
    API = DFS --dfs.pool=hennecke_pool01 --dfs.cont=cont_rf0 --dfs.dir_oclass=S1 --dfs.oclass=SX
    [ior-hard]
    API = DFS --dfs.pool=hennecke_pool01 --dfs.cont=cont_rf0 --dfs.dir_oclass=S1 --dfs.oclass=SX
    [mdtest-easy]
    API = DFS --dfs.pool=hennecke_pool01 --dfs.cont=cont_rf0 --dfs.dir_oclass=SX --dfs.oclass=S1
    [mdtest-hard]
    API = DFS --dfs.pool=hennecke_pool01 --dfs.cont=cont_rf0 --dfs.dir_oclass=SX --dfs.oclass=S1
```
...
boundaries of the servers, clients, and InfiniBand NDR network.
